...
export type WorldArtifactEditorProps = {
...
export function WorldArtifactEditor({
    ...
    useEffect(() => {
        ...
        startTransition(async () => {
            ...
            const nextAvailableArtifacts = await findWorldArtifacts({ worldId })
            ...
            if (state.mode === 'create') {
                setArtifact(null)
                setArtifactType(state.artifactType ?? 'World')
                setParentArtifactIds({})
                setTitle('')
                setMapImageUrl('')
    ...
    }, [findWorldArtifactById, findWorldArtifacts, state, worldId])
