...
describe('Workspace', () => {
    ...
    it('renders the shell and default border layout regions', () => {
        ...
        expect(screen.getByLabelText('Choose visible panels')).toBeTruthy()
        ...
        const actorsPanel = addPanelMock.mock.calls.find(
            ([panel]) => panel.id === 'actors'
        )?.[0]
        expect(actorsPanel).toEqual(
            expect.objectContaining({
                id: 'actors',
                component: 'actors',
                title: 'Actors',
                position: { referencePanel: 'world' },
            })
        )
        expect(addPanelMock).toHaveBeenCalledWith(
            expect.objectContaining({
